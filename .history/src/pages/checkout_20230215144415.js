...
const checkout = () => {
  ...
  const createCheckout = async () => {
    ...
    const checkoutSessionsCollection = collection(db, 'checkoutSessions', checkoutSession.data.email);

    try {
      const docRef = await addDoc(checkoutSessionsCollection, checkoutSession.data);
      console.log('Checkout session added with ID: ', docRef.id);
    } catch (error) {
      console.error('Error adding checkout session to Firestore: ', error);
    }
