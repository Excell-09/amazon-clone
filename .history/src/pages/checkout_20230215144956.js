...
const checkout = () => {
  ...
  const createCheckout = async () => {
    ...
    </html>(db, 'users', session.user.uid);

    const orderHistoryCollectionRef = collection(userDocRef, 'orderHistory');

    try {
      const docRef = await addDoc(orderHistoryCollectionRef, checkoutSession.data);
      console.log('Checkout session berhasil ditambahkan ke orderHistory dengan ID: ', docRef.id);
    } catch (error) {
      console.error('Error saat menambahkan checkout session ke orderHistory: ', error);
    }
