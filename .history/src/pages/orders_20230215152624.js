...
export async function getServerSideProps(context) {
  ...
  const stipeOrders = await db.collection('users').doc(session.user.email).collection('orderHistory').orderBy('timestamp', 'decs').get();


  const orders = await Promise.all(str)
